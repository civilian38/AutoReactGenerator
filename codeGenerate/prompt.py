...
folder_apidoc_init_message="이 섹션의 API 명세는 클라이언트가 현재 페이지를 구현하는데 사용하려고 하는 API가 기술되어 있습니다."
folder_generate_final_message="위 정보를 종합하여 생성해야 할 폴더 목록을 작성하시오. 그러나 되도록 프로젝트의 구조가 너무 복잡해지지 않도록 가능한 적은 개수의 폴더를 만드는 것을 권장합니다. 또한 새로운 폴더를 만들지 않아도 된다는 것을 기억하십시오."